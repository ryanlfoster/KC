			...
			if ($crActions->isGivePointsAction ( $rule->getPointsAction () )) {
				...
				if (Mage::helper ( 'rewards/config' )->calcCartPointsAfterDiscount ()){					
					...
					if ($quote){
						...
						if($address){
                            //@triley -a 19/03/12: Jira fix 1210
                            //revert the cart discounts to the base currency for price calculations
							$cartDiscounts = Mage::helper('rewards/price')->getReversedCurrencyPrice($address->getDiscountAmount());
