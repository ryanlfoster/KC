...
class TBT_Rewardssocial_Model_Facebook_Like_Validator extends TBT_Rewards_Model_Special_Validator {
        ...
        try {
            ...
            if($wait_time > 0) {
                Mage::getSingleton('core/session')->addError(
                    Mage::helper('rewardssocial')->__('Please wait %s second(s) before liking another page if you want to be rewarded.', $wait_time));
                return $this;
            }
            ...
            if( $this->maxLikesReached($customer) ) {
                Mage::getSingleton('core/session')->addError(
                    Mage::helper('rewardssocial')->__("You've reached the Facebook like rewards limit for today (%s)", $max_likes));
                return $this;
            }
            ...
            Mage::getSingleton('core/session')->addError(Mage::helper('rewardssocial')->__('Could not reward you for your Facebook mention.'));
            Mage::logException($e);
        }
    ...
    protected function _cancelUnlikedTransfers($like_model) {
        ...
        foreach($transfer_col as &$transfer) {
            ...
            Mage::log($transfer->getData());

            /* BEGIN update to handle approved status transfers @triley -a 16/03/12: */
            
            //check status of transfer - if approved then don't attempt to cancel and show a different message
            if ($transfer->getStatus() == TBT_Rewards_Model_Transfer_Status::STATUS_APPROVED) {
                Mage::getSingleton('core/session')->addSuccess(
                    Mage::helper('rewardssocial')->__('You have unliked this item.')
                );
            } else {
            ...
            /* END update to handle approved status transfers */
